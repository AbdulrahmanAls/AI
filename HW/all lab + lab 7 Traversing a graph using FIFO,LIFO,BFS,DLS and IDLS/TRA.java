...
public class TRA {
    ...
    while(!stack1.empty()){
        ...
        visit[currPos] = true;
        if(currPos == destination){
            addToSta+="This is the destination " +retCity(currPos)+" level:"+levels1[currPos] +" \t;";
            break; 
        }
            ...
            if(G[currPos][i]==true)
                {
                    if(visit [i] == false){
                        if(current_level <= limit){
                            ...
                            stack1.push(i);//The connected cities are added to the statck  
                            addToSta+=retCity(i)+"\t";
                            levels1 [i] = current_level + 1 ;
                            if(i == destination)
                            break; 
                        }
                    }
